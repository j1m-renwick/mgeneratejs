 ...
 */
var callOperator = function(op, opts, ctx) {
  opts = opts || {};
  ...
  if (_.includes(opNames, op)) {
    return operators[op](evalTemplate, opts, ctx);
  }
...
/**
 * Entry point from the binary.
 *
 * evaluates and stores context data, and then uses it to evaluate the json template.
 *
 * @param  {count} count        the number of times the generator has been invoked
 * @param  {object} template    template to evaluate
 * @param  {object} ctx         template context
 * @return {any}                return value after template is evaluated
 */
processTemplate = function(count, template, ctx) {
  templateContext = { i: count };
  _.mapValues(ctx, function(obj, key) {
    store(key, obj);
  });
...
var store = function(key, val) {
  var value = evalTemplate(val, true);
...
evalValue = function(template) {
  ...
  if (_.isPlainObject(template)) {
    ...
    if (_.startsWith(op, '$')) {
      ...
      return callOperator(op, options, templateContext);
    }
    return evalObject(template);
  }
